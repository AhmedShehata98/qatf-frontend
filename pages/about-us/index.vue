  ...
  >
    ...
    >
      ...
      >
        ...
        <div class="w-full flex items-center justify-start gap-4 mt-12">
          <span
            class="min-w-[calc(50%-1rem)] min-h-[146px] relative flex items-center justify-center p-5 bg-black rounded-[20px]"
          >
            <span
              class="absolute w-full top-0 pe-2 left-0 rounded-tr-[20px] overflow-hidden flex items-center justify-between"
            >
              <span
                class="px-4 py-3 md:px-6 md:py-5 text-white max-md:text-xs bg-zinc-900 flex items-center justify-center rounded-bl-full"
              >
                <p>{{ data?.aboutUs.secondaryBoxTitle }}</p>
              </span>
              <span
                class="flex size-8 md:size-10 p-1.5 md:p-2 relative bg-white rounded-full"
              >
                <span class="size-full rounded-full flex bg-black"></span>
              </span>
            </span>
            <h3
          ...
          >
            <span
              class="absolute w-full top-0 ps-2 left-0 rounded-tl-[20px] overflow-hidden flex items-center justify-between flex-row-reverse"
            >
              <span
                class="px-4 py-3 md:px-6 md:py-5 text-white max-md:text-xs bg-primary brightness-110 flex items-center justify-center rounded-br-full"
              >
                <p>{{ data?.aboutUs.primaryBoxTitle }}</p>
              </span>
              <span
                class="flex size-8 md:size-10 p-1.5 md:p-2 relative bg-white rounded-full"
              >
                <span class="size-full rounded-full flex bg-primary"></span>
              </span>
            </span>
            <h3
              class="text-white mt-14 md:mt-24 pb-5 tablet:px-8 text-base tablet:text-xl font-bold max-w-full overflow-hidden truncate"
    ...
    <div class="w-full flex flex-col items-center justify-center mt-16">
      ...
      >
        <li
          v-for="reason in data.aboutUs.whyItems"
          :key="reason.id"
...
const descriptionSpacialWords = computed(() => {
  const description = data?.value?.aboutUs?.descriptionText;
  ...
  return matches.map((match) => match[1].trim()).map((i) => i.split(" "))[0];
});
</script>
